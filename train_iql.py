...
def train(env, config):
    ...
    agent = Agent(state_size=200, action_size=4,  config=config) 
...
def eval_policy(env, config):
    ...
    for t in range(1000,30001, 1000):
        print(t)
        path = "results_40k/models/{}-".format(t)
        agent.load(path)
...
def create_vid(env, config):
    ...
    save_models_path =  str(config["locexp"])
    agent = Agent(state_size=200, action_size=4,  config=config) 
    path = "results_10k/models/{}-".format(13000)
    agent.load(path)
    agent.eval_policy(record=True, eval_episodes=10, steps=0)
